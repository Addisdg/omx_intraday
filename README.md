...
Provider outputs also carry in-memory metadata through `DataFrame.attrs`, including provider name, source, symbol, interval, period, retrieval time, row count, adjustment flag, and warnings. `analyze_dataframe()` returns this as `provider_metadata`. Cached CSV loads are marked with `source=cache`, and cache writes include a `.metadata.json` sidecar so original provider provenance can survive reloads.
